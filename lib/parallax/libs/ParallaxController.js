...
function ParallaxController () {
  ...
  function _handleScroll () {
    ...
    // Save current scroll
    scrollX = window.pageXOffset; // Supports IE 9 and up.
  ...
  function _setParallaxStyles (element) {
    ...
    const left = element.attributes.left - scrollY
    const { totalDistX, totalDistY } = element.attributes

    // Percent the element has moved based on current and total distance to move
    const percentScrolledY = (top * -1 + windowHeight) / totalDistY * 100
    const percentScrolledX = (left * -1 + windowWidth) / totalDistX * 100

    // Scale percentScrolledY to min/max percent determined by offset props
    const { slowerScrollRate } = element.props
