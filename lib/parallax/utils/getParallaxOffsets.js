...
export default function getParallaxOffsets (
  ...
  if (slowerScrollRate) {
    x = scaleBetween(percentMoved.x, xMin.value, xMax.value, 0, 100)
    y = scaleBetween(percentMoved.y, yMin.value, yMax.value, 0, 100)
  } else {
    // flipped max/min
    x = scaleBetween(percentMoved.x, xMax.value, xMin.value, 0, 100)
    y = scaleBetween(percentMoved.y, yMax.value, yMin.value, 0, 100)
  }
